...
export const renderSvgToPng = async (svg, { width, height }) => {
  const canvas = document.createElement('canvas');
  canvas.width = width;
  canvas.height = height;
  const ctx = canvas.getContext('2d');
  if (!ctx) {
    throw new Error('Canvas export failed; 2D context is unavailable.');
  }
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      image.onerror = () => reject(new Error('Canvas export failed; SVG image could not be loaded.'));
      image.src = objectUrl;
    });
    ...
    const pngBlob = await new Promise((resolve, reject) => {
      canvas.toBlob((blob) => {
        if (!blob) {
          reject(new Error('Canvas export failed; no blob returned.'));
          return;
        }
        resolve(blob);
      }, 'image/png');
    });
    return blobToUint8(pngBlob);
  } finally {
...
export const renderPaletteCardPng = async (theme) => (
  renderSvgToPng(buildPaletteCardSvg(theme), { width: 1200, height: 800 })
);

export const renderStripPng = async (theme) => (
  renderSvgToPng(buildStripSvg(theme), { width: 1500, height: 420 })
);
